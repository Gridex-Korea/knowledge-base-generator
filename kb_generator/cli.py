...
- `project/roadmap.md` — 구축 로드맵
- `project/evidence-gaps.md` — 확인되지 않은 근거 공백
- `gcp/` — GCP 자동화/RAG 확장 구성
...
- `project/evidence-gaps.md`에서 미확인 근거를 관리합니다.
"""
...
def render_gcp_config(spec: ProjectSpec) -> str:
    return f"""version: 1
...
canonical_repository: "REPLACE_WITH_OWNER/{spec.slug}"
vertex_ai:
...
def write_builtin_starter(output: Path, spec: ProjectSpec) -> None:
    files = {
        ...
        output / "attachments" / "README.md": "# Attachments\n\n재배포 권한이 확인된 이미지·도표만 저장합니다. 공식 원문 PDF는 기본적으로 Source Registry의 URL로 관리합니다.\n",
    }
...
def generate(spec: ProjectSpec, output: Path, template_root: Path | None = None, force: bool = False) -> list[Path]:
    if output.exists() and any(output.iterdir()) and not force:
        raise FileExistsError(f"output directory is not empty: {output}; use --force to merge")
    ...
    files = {
        ...
        output / "project.yaml": render_project_yaml(spec),
        output / "project" / "roadmap.md": render_roadmap(spec),
        output / "project" / "evidence-gaps.md": render_evidence_gaps(),
        output / "gcp" / "README.md": render_gcp_readme(spec),
        output / "gcp" / "config.yaml": render_gcp_config(spec),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--private", action="store_true", help="Mark the generated KB as private")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    if args.json:
        print(json.dumps({"output": str(output), "project": asdict(spec)}, ensure_ascii=False, indent=2))
    else:
